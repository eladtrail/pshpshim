...
function renderProductCard(p) {
  ...
  const ctaBtn = isAffiliate
    ? `<a href="${p.link}" target="_blank" rel="noopener sponsored" class="btn btn-primary btn-buy" onclick="event.stopPropagation()">
         <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
         <span data-i18n="btn_buy">${t('btn_buy')}</span>
       </a>`
    : `<button onclick="event.stopPropagation();contactProduct('${p.id}')" class="btn btn-whatsapp btn-buy">
  ...
  return `
    <article class="product-card reveal" data-id="${p.id}" data-type="${p.type}"
             onclick="location.href='/product?id=${p.id}'" style="cursor:pointer;"
             role="link" tabindex="0" onkeydown="if(event.key==='Enter')location.href='/product?id=${p.id}'"
             aria-label="פרטי המוצר ${name}">
